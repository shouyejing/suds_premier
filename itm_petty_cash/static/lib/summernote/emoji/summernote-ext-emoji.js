...
odoo.define('itm_petty_cash.summernote_ext_emoji', function (require) {
'use strict';
	...
	for (var j = 0; j < chunks.length; j++) {
		...
		for (var i = 0; i < chunks[j].length; i++) {
			...
			emoList += '<div class="col-xs-3">'
					...
					+ '/itm_petty_cash/static/lib/summernote/emoji/pngs/'
					+ emo
...
$.summernote.addPlugin({
	...
	events : {
		selectEmoji : function(event, editor, layoutInfo, value) {
			...
			var img = new Image();
            img.src = '/itm_petty_cash/static/lib/summernote/emoji/pngs/' + value + '.png';
            img.alt = value;
