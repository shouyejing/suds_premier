...
class SprogroupPurchaseRequest(models.Model):
    ...
    def button_to_approve(self):
        if not self.assigned_to:
            raise UserError(_("Please specify the approver for this request."))
    ...
    @api.multi
    def name_get(self):
        res = super(SprogroupPurchaseRequest, self).name_get()
        data = []
        for i in self:
            display_value = ''
...
class SprogroupPurchaseRequestLine(models.Model):
    ...
    description = fields.Text(related='request_id.description',
                              ...
                              store=True)
    date_required = fields.Date(string='Due Date', required=True,
                                track_visibility='onchange',
                                default=fields.Date.context_today)
