...
class RoyaltyFeetoInvoice(models.TransientModel):
    ...
    type = fields.Char(default='out_invoice')
    journal_id = fields.Many2one('account.journal', string='Journal',
        default=_default_journal_invoice,
        domain="[('type', '=', 'out_invoice'), ('company_id', '=', company_id)]")
    @api.model
    ...
    def _default_account(self):
        if self._context.get('journal_id'):
            ...
            if self._context.get('type') in ('out_invoice'):
    ...
    def _prepare_royalty_invoice_line_royalty_fee(self, line):
        return {
                ...
                'month': line.month.id,
                'quantity': 1,
                'name': line.month.id + ' Royalty Fee',
    ...
    def _prepare_royalty_invoice_line_swp(self, line):
        return {
                ...
                'month': line.month.id,
                'quantity': 1,
                'name': line.month.id + ' System Wide Payment Fee',
                'price_unit':line.swp,
