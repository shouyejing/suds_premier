...
class SalesOrderLineAquion(models.Model):
    ...
    @api.depends('date_pf_pickup', 'shift')
    def _create_section(self):
        for i in self:
            section_shift = self.env['sale.layout_category'].create({'name': str(i.date_of_pickup) + i.shift}).id
        return self.write({'layout_category_id': section_shift})
