...
class PurchaseReportXlsx(models.AbstractModel):
    _name = 'report.purchase_history_report'
    ...
    def generate_xlsx_report(self, workbook, data, line):
        ...
        # TOP COLUMN HEADERS
        top_column = ["Vendor", "Invoice Purchase Document",
                      "Invoice Date", "Due Date", "Responsible", "Product",
                      "Product Category", "Account", "Analytic Account",
                      "Quantity", "Sales Price", "Cost Price","Unit Price","Subtotal","Status"]
    ...
    # =====================================ALL CELL PRINTING GOES HERE===================================================
        ...
        if selected_invoice_line_records[:1]:
            for i in selected_invoice_line_records:
                ...
                data_columns = [
                    i.invoice_id.partner_id.name.upper(),  # VENDOR
                    i.invoice_id.invoice_purchase_document.name or "None",  # INVOICE PURCHASE DOCUMENT
                    i.invoice_id.date_invoice or "None",  # INVOICE DATE
                    i.invoice_id.date_due or "None",  # DUE DATE
                    i.invoice_id.partner_id.user_id.name or "None",  # RESPONSIBLE
                    i.product_id.name_get()[0][1] if i.product_id.name_get(
                    ) == True else i.name,  # PRODUCT OR DESCRIPTION
                    i.product_id.categ_id.name or "None",  # PRODUCT CATEGORY
                    i.account_id.name_get()[0][1],  # ACCOUNT
                    i.account_analytic_id.name or "None",  # ANALYTIC ACCOUNT
                    i.quantity,  # QUANTITY
                    i.product_id.lst_price,  # SALE PRICE
    ...
    # # =====================================CELL FORMULAS FOR TAX TYPE "Y"============================================
        ...
        if selected_invoice_line_records[:1]:
            # # TOTAL
            # sheet.write(
            #     counter+1, 13, "TOTALS", total_sales_format)
            # AMOUNT UNTAXED
            sheet.write(
                counter+2, 13, "AMOUNT UNTAXED", total_amount_format)
            sheet.write(
                counter+2, 14, sum(amount_untaxed), total_amount_format)
            # AMOUNT TAX
            sheet.write(
                counter+4, 13, "TAX", total_amount_format)
            sheet.write(
                counter+4, 14, sum(amount_tax), total_sales_format)
            # AMOUNT TOTAL
            sheet.write(
                counter+6, 13, "TOTAL AMOUNT", total_amount_format)
            sheet.write(
                counter+6, 14, sum(amount_total), total_sales_format)
            # AMOUNT DUE
            sheet.write(
                counter+8, 13, "AMOUNT DUE", total_amount_format)
            sheet.write(
                counter+8, 14, sum(amount_due), total_sales_format)
